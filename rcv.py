...
def infer_consumer(pack_queue: deque, pack_size:int, condition: threading.Condition, terminate_event: threading.Event, snapshot_path: Path, output_csv_file: Path):
...
def main(fifo_path: Path, pack_size: int, snapshot_path: Path, output_csv_file: Path):
    ...
    try:
        ...
        t2 = threading.Thread(target=infer_consumer, args=(pack_queue, pack_size, condition, terminate_event, snapshot_path, output_csv_file))
        t1.start()
        t2.start()
        t1.join()
        t2.join()
