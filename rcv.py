    ...
    try:
        while not terminate_event.is_set():
            try:
                ...
                signal_termination(condition, terminate_event, f"FIFO read error: {ex}. Exiting.")
                break

            #print(f"CHUNKSIZE: {len(chunk)}")
            if not chunk:
                signal_termination(condition, terminate_event, "Writer has closed the FIFO. Exiting.")
                break
            ...
            while '\n' in buffer:
                ...
                if line.strip():
                    try:
                        ...
                        signal_termination(condition, terminate_event, f"Malformed JSON from FIFO: {ex}. Exiting.")
                        return
