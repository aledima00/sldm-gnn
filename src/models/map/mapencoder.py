...
class MapEncoder(_nn.Module):
    def __init__(self, map_float_features:_tch.Tensor, map_bool_features:_tch.Tensor, lane_type_cats:_tch.Tensor, graph_edge_indexes:_tch.Tensor,*,lane_embed_dim:int=2,sage_hidden_dims:list[int]=[8,8],dropout,negative_slope):
        ...
        self.register_buffer('map_float_features', map_all_float_features, persistent=True)
        self.register_buffer('lane_type_cats', lane_type_cats, persistent=True)
        self.register_buffer('graph_edge_indexes', graph_edge_indexes, persistent=True)

        self.lane_embedding = _nn.Embedding(
            num_embeddings = int(_tch.max(lane_type_cats).item()) + 1,
            embedding_dim = lane_embed_dim
        )
        input_dim = map_all_float_features.shape[1] + lane_embed_dim
